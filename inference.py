...
# Date: 17 August 2021
# Additional Copyright: Khalifa University Abu Dhabi
...
def create_test_csv_json(data_p, region_id, metadata_path, product='CMA', 
                         n_bins=96, n_preds=32, n_files=4, competition='stage-1'):
...
# 3. load data & model
# ------------
def get_data_iterator(region_id, data_path, splits_path, data_split='test', collapse_time=False, 
                      batch_size=32, shuffle=False, num_workers=0):
    """ Creates an iterator for data in region 'region_id' for the days in `splits_path`
    """
    params = cf.get_params(region_id=region_id, competition='stage-1',
                           collapse_time=collapse_time, use_static=False,
                           use_all_variables=False,use_cloud_type=False, use_time_slot=False)
    ...
    ds = create_dataset('test', params['data_params'], precision=32, populate_mask=True)
...
def load_model_and_weights(root_to_ckp, device=None):
    """ Loads a model as we basically have a single model trained on all the available data
    """
    # regions where we have trained models
    #\logs\ALL_real_swinencoder3d_688080\20210630T224355\checkpoints\epoch=58-val_loss=0.029748.ckpt
    checkpoint_path = f"{root_to_ckp}/20210630T224355/checkpoints/epoch=58-val_loss=0.029748.ckpt"
...
def get_preds(model, batch, device=None):
    ...
    day_in_year = metadata['out']['day_in_year'][0][0].item()
    ...
    y_hat = torch.reshape(y_hat, (-1, 32, 4, 256, 256))
    y_hat = y_hat.data.cpu().numpy()
...
def inference(data_p, region, weights, output, gpu_id):
    ...
    model = load_model_and_weights(weights, device=gpu_id)

    # ------------
    # C. Predict and save the predictions
    # ------------
    predictions_per_day(test_dates, model, ds_iterator, gpu_id, out_path, params['data_params'])
